...
from utils.styles import PLOTLY, C, STAGE_COLORS, ABCXYZ_COLORS, kpi, sec_hdr, stage_badge, abc_badge, xyz_badge, badge
...
def _bar(x, y, name, color, xt="", yt="", hover=None):
    return go.Bar(
        x=x, y=y, name=name,
        marker_color=color, marker_line_width=0, opacity=0.85,
        hovertemplate=hover or f"<b>%{{x}}</b><br>{name}: %{{y:,.1f}}<extra></extra>",
    ), xt, yt
...
def render(
    ...
    st.markdown('<div class="sec-hdr">Inventory Intelligence</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f'<b>Forecast horizon:</b> {horizon_label}'
        f' &nbsp;|&nbsp; Products: <b>{len(master):,}</b>'
        f' &nbsp;|&nbsp; Model: LightGBM quantile regression ({n_lgb} SKUs) · '
        f'Simple average fallback ({n_avg} SKUs)'
        f'<br><span style="font-size:11px;opacity:.75">'
        f'All columns labelled <b>Pred</b> are model forecasts — not actuals.</span>'
        f'</div>',
    ...
    with c1: kpi("Products Tracked",    f"{len(master):,}")
    with c2: kpi("Pred Units (mid)",    f"{int(master['pred_qty_mid'].sum()):,}",
                 sub=f"Next {horizon_label}")
    with c3: kpi("A-Class Products",    f"{(master['abc']=='A').sum()}")
    with c4: kpi("Trending Now",        f"{(master['stage']=='trending').sum()}",
                 sub=f"Fast movers: {(master['stage']=='fast_mover').sum()}")
    with c5:
        dead_a = (master[(master["stage"]=="dead") & (master["abc"]=="A")]).shape[0]
        color  = "red" if dead_a > 0 else "green"
        kpi("Dead A-Class",  f"{dead_a}",
            sub="Urgent review needed" if dead_a>0 else "None — all healthy")
    ...
    if not dead_ab.empty:
        ...
        st.markdown(f'<div class="alert-box">Dead A/B products ({len(dead_ab)}): {names}</div>',
                    unsafe_allow_html=True)
    if not slow_ab.empty:
        st.markdown(f'<div class="warn-box">Slow movers in A/B class: {len(slow_ab)} products — consider promotions or clearance</div>',
                    unsafe_allow_html=True)
    if not trend.empty:
        names = ", ".join(trend["name"].head(3).str.title().tolist())
        st.markdown(f'<div class="success-box">Trending products ({len(trend)}): {names} — ensure stock coverage</div>',
                    unsafe_allow_html=True)
...
def _tab_master(master, has_rev, horizon_label):
    ...
    st.markdown(f'<div style="font-size:12px;color:#5C6380;margin-bottom:8px">Showing {len(view):,} of {len(master):,} products</div>', unsafe_allow_html=True)

    # Prediction range chart — top 20
    if len(view) > 0:
        ...
        fig.add_trace(go.Bar(
            ...
            hovertemplate="<b>%{y}</b><br>Predicted units: %{x:.0f}<extra></extra>",
        ))
        fig.update_layout(**_L(
            title=f"[Forecast] Next {horizon_label} — Predicted Units (Top 20, with 25/75 quantile range)",
            height=520,
            yaxis=dict(autorange="reversed", **PLOTLY["yaxis"]),
            xaxis_title="Predicted Units (Forecast)",
            yaxis_title="Product",
        ))
    ...
    if has_rev: dcols.append("pred_revenue_mid")
    ...
    st.write(dd.to_html(escape=False,index=False), unsafe_allow_html=True)
    st.markdown("---")
    st.download_button(
...
def _tab_action_queue(master, horizon_label):
    ...
    # Dead A/B products
    with c1:
        st.markdown("**Dead A/B-class products** (no sales 60+ days)")
        dead = master[(master["stage"]=="dead") & master["abc"].isin(["A","B"])].copy()
        if dead.empty:
            st.markdown('<div class="success-box">No dead A/B products. Good.</div>', unsafe_allow_html=True)
        else:
            d = dead[["name","abc","last_6m_qty","days_since_sale","true_avg_6m","margin_pct"]].copy()
            d["abc"] = d["abc"].apply(abc_badge)
            d = d.rename(columns={"name":"Product","abc":"ABC","last_6m_qty":"6M Qty",
                                   "days_since_sale":"Days Dormant","true_avg_6m":"Avg/Mo","margin_pct":"Margin %"})
            st.write(d.to_html(escape=False,index=False), unsafe_allow_html=True)
            st.download_button("Export Dead A/B List",
                dead.to_csv(index=False).encode("utf-8"),
                f"dead_ab_{datetime.date.today()}.csv","text/csv", key="dl_dead")

    # Trending + fast movers needing coverage
    with c2:
        st.markdown("**Trending / Fast movers** (ensure stock coverage)")
        hot = master[master["stage"].isin(["trending","fast_mover"])].copy()
        hot = hot.sort_values("pred_qty_mid", ascending=False)
        if hot.empty:
            st.markdown('<div class="info-box">No trending products detected currently.</div>', unsafe_allow_html=True)
        else:
            h = hot.head(15)[["name","abc","stage","last_month_qty","pred_qty_low","pred_qty_mid","pred_qty_high"]].copy()
            h["abc"]   = h["abc"].apply(abc_badge)
            h["stage"] = h["stage"].apply(stage_badge)
            h = h.rename(columns={"name":"Product","abc":"ABC","stage":"Stage",
                                   "last_month_qty":"Last Mo (Actual)",
                                   "pred_qty_low":"★ Pred Low","pred_qty_mid":"★ Pred Mid","pred_qty_high":"★ Pred High"})
            st.write(h.to_html(escape=False,index=False), unsafe_allow_html=True)

    st.markdown("---")

    # Slow movers in A/B
    st.markdown("**Slow movers in A/B class** (below-average velocity, consider promotions)")
    ...
    if slow.empty:
        ...
        s = slow[["name","abc","xyz","last_month_qty","true_avg_6m","margin_pct","days_since_sale","pred_qty_mid"]].copy()
        ...
        st.write(s.to_html(escape=False,index=False), unsafe_allow_html=True)
...
def _tab_category(cat_view, monthly):
    ...
    # Category monthly revenue stacked bar
    cat_rev = cat_view.pivot_table(index="month_str", columns="category",
                                    values="qty_sold", aggfunc="sum", fill_value=0).reset_index()
    fig = go.Figure()
    palette = px.colors.qualitative.Set2
    for i, cat in enumerate([c for c in cats if c in cat_rev.columns]):
        fig.add_trace(go.Bar(
            x=cat_rev["month_str"], y=cat_rev[cat],
            name=cat, marker_color=palette[i % len(palette)], opacity=0.85,
    ...
    # Revenue vs Margin per category
    ...
    cat_tot["margin_pct"] = (cat_tot["total_profit"]/cat_tot["total_revenue"].replace(0,np.nan)*100).round(1).fillna(0)
    ...
    with cr:
        ...
        fig3.add_trace(go.Bar(
            ...
            name="Gross Margin %",
            marker_color=[C["green"] if v>=25 else C["yellow"] if v>=15 else C["red"]
                          for v in cat_tot["margin_pct"]],
            opacity=0.85,
            hovertemplate="<b>%{x}</b><br>Margin: %{y:.1f}%<extra></extra>",
        ))
        fig3.add_hline(y=cat_tot["margin_pct"].mean(), line_dash="dot",
                       line_color=C["faint"],
                       annotation_text=f"Avg {cat_tot['margin_pct'].mean():.1f}%",
    ...
    if not sub.empty:
        ...
        fig4.update_layout(**_L(
            ...
            yaxis2=dict(
                ...
                title="Revenue",
                title_font=dict(color=C["green"], size=12),
                showgrid=False,
            ),
        ))
...
def _tab_model(master, monthly, feat_imp):
    ...
    with cl:
        if "abcxyz" in master.columns:
            ...
            colors_bar = [ABCXYZ_COLORS.get(c, C["gray"]) for c in all_combos]
            ...
            fig.update_layout(**_L(
                title="ABC×XYZ Portfolio Matrix (Product Count)",
                height=300,
